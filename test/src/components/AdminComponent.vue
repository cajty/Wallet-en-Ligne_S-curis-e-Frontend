    ...
    <div v-if="accounts" class="mt-4">
      <div v-for="account in accounts" :key="account.id" class="card mb-4">
        <div class="card-body">
          ...
          <div class="mt-3">
            <button @click="performTransaction(account.id, 'transaction')" class="btn btn-primary me-2">Transaction</button>
            <button @click="performTransaction(account.id, 'send')" class="btn btn-primary">Send</button>
          </div>
        </div>
      </div>
    </div>
  ...
  export default {
    ...
    methods: {
      getAccount() {
        ...
        const token = 'Bearer 27|GMwQqEPAVovqXJf80Yz4TM1LBeDWYZy5OA7qur7Gffc31926';
